...
import { Shield, ArrowLeft, ArrowRight, Plus, X, Play, Download, Loader2, CheckCircle, AlertCircle, FileText, Layers } from "lucide-react";
import { cn } from "@/lib/utils";
import { tools } from "@/data/tools";

const API = import.meta.env.VITE_API_URL || "";

// Pipeline-compatible tools (PDF → PDF)
const pipelineTools = tools
    .filter(t => t.accepts === ".pdf" && t.outputLabel?.endsWith(".pdf"))
    .map(t => ({ slug: t.slug, name: t.name, icon: t.icon }));
...
export default function PipelinePage() {
    ...
    const filteredTools = stepSearch.trim()
    ...
    const runPipeline = async () => {
        ...
        for (let i = 0; i < steps.length; i++) {
            ...
            try {
                ...
                const resp = await fetch(`${API}/api/${steps[i].tool.slug}`, {
                    method: "POST",
                    body: formData,
                });
                ...
                if (!resp.ok) {
                    ...
                    throw new Error(`Step ${i + 1} (${steps[i].tool.name}) failed: ${resp.status}`);
                }
        ...
        <div className="min-h-screen bg-background">
            ...
            <main className="mx-auto max-w-[900px] px-5 py-10">
                ...
                <p className="text-sm text-muted-foreground mb-8">
                    Chain multiple PDF tools together. Your file goes through each step in sequence — e.g. compress → watermark → protect.
                </p>
                ...
                <div className="mb-8">
                    ...
                    {!file ? (
                        ...
                        <div className="flex items-center gap-3 px-4 py-3 rounded-xl border border-border/50 bg-card/40">
                            ...
                            <button onClick={() => { setFile(null); setResultUrl(null); }} className="text-muted-foreground/40 hover:text-muted-foreground">
                                <X size={14} />
                            </button>
                        </div>
                    )}
                                ...
                                <div key={i} className={cn(
                                    ...
                                    {!processing && (
                                        <div className="flex items-center gap-1">
                                            {i > 0 && (
                                                <button onClick={() => moveStep(i, -1)} className="text-muted-foreground/30 hover:text-muted-foreground p-0.5" title="Move up">↑</button>
                                            )}
                                            {i < steps.length - 1 && (
                                                <button onClick={() => moveStep(i, 1)} className="text-muted-foreground/30 hover:text-muted-foreground p-0.5" title="Move down">↓</button>
                                            )}
                                ...
                                <div className="mt-3 border border-border/50 bg-card/60 rounded-xl p-3">
                                    ...
                                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-1 max-h-[180px] overflow-y-auto">
                                        {filteredTools.slice(0, 18).map(t => {
                                            const Ic = t.icon;
                                            return (
                                                <button key={t.slug} onClick={() => addStep(t)}
                                    ...
                                    <button onClick={() => { setAddingStep(false); setStepSearch(""); }}
                                        className="mt-2 text-[12px] text-muted-foreground hover:text-foreground">Cancel</button>
                                </div>
                            ) : (
                                <button
